...
namespace CGL { namespace SceneObjects {
  ...
  void EnvironmentLight::init() {
    ...
    for (int j = 0; j < h; ++j) {
      ...
      conds_y[j * w] = pdf_envmap[j * w] / marginal_sum;
      for(int i = 1; i < w; ++i)
        conds_y[w * j + i] = conds_y[w * j + i - 1] + pdf_envmap[w * j + i] / marginal_sum;
  ...
  Vector3D EnvironmentLight::sample_L(const Vector3D p, Vector3D* wi,
    ...
    // *distToLight = INF_D;
    // *pdf = 1.0 / (4.0 * PI);
    // Vector2D angle = dir_to_theta_phi(*wi);
    // Vector2D coords = theta_phi_to_xy(angle);
    // return bilerp(coords);  

    // important
    int w = envMap->w, h = envMap->h;
    Vector2D sect = sampler_uniform2d.get_sample();
    int j = std::upper_bound(marginal_y, marginal_y + h, sect[0]) - marginal_y;
    int i = std::upper_bound(conds_y + j * w, conds_y + (j+1) * w, sect[1]) - (conds_y + j * w);
    Vector2D coord = Vector2D(i, j);
    *wi = theta_phi_to_dir(xy_to_theta_phi(coord));
    *distToLight = INF_D;
    *pdf = pdf_envmap[j * w + i] * (w*h) / (2*PI*PI*sin_theta(*wi));


    return envMap->data[j*w + i];
  }
