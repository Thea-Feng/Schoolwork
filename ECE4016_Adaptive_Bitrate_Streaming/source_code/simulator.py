...
if __name__ == "__main__":
    ...
    while chunk:
        ...
        fg = pre_buf_occ <= buf_occ['current']
        av_bitrates = prep_bitrates(manifest["Available_Bitrates"],chunk)
        chunk_arg = prep_chunk(chunks_remaining, manifest, chunknum)

        # print("This chunk num is ", chunknum)
        # print("m_band: ", m_band)
        # print("buf_occ: ", buf_occ)
        # print("avg_bitrates: ", av_bitrates)
        # print("chunk_arg: ", chunk_arg)


        #send info to student, get response
        chosen_bitrate, B_delay = simulator_comm.send_req_json(m_band, prev_throughput, buf_occ, av_bitrates, current_time, chunk_arg, rebuff_time, pref_bitrate, chunk_item, fg)

        # print("my result: ", chosen_bitrate, B_delay)
        ...
        pre_buf_occ = buf_occ['current']
        prev_throughput = (stu_chunk_size * 8) / time_elapsed
        current_time += time_elapsed
        chunks_remaining -= 1

        logger.log_rebuffer(current_time - rebuff_time, rebuff_time)
        current_time += B_delay
        chunk_item.append({"id":chunknum, "begin_time":current_time-time_elapsed, "end_time": current_time, "avg_throughput":prev_throughput})

        #log actions

        # print("After example")    
        # print("student_chunk_size: ", stu_chunk_size)
        # print("time_elapse: ", time_elapsed)
        # print("prev_throughput: ", prev_throughput)
        # print("current time: ",current_time)
        # print("rebuffer time: ", rebuff_time)
        # print("buffer occupy: ", buf_occ)
        # print()
        #get next chunk
