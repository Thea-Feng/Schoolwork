...
__device__ __managed__ URL_node pool[STORAGE_SIZE];
const int SWAP_TABLE_SIZE = 5120;
__device__ __managed__ u32 swap_table[SWAP_TABLE_SIZE];
...
__global__ void mykernel(int input_size) {
  ...
  vm_init(&vm, data, storage, pt, &pagefault_num, PAGE_SIZE,
          ...
          PHYSICAL_MEM_SIZE / PAGE_SIZE, swap_table, pool, thread_id);
